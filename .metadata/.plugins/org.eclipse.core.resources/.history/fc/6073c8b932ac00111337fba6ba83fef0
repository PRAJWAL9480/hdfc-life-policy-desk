...
public class CourseController {
    ...
    public ResponseEntity<List<CourseResponseDto>> getCoursesByTrainer(
            @PathVariable String trainerName) {

        return ResponseEntity.ok(
                courseService.getCoursesByTrainer(trainerName));
    }

    // Get courses with fees less than amount
    @GetMapping("/fees-less-than/{amount}")
    public ResponseEntity<List<CourseResponseDto>> getCoursesByFeesLessThan(
            @PathVariable Double amount) {

        return ResponseEntity.ok(
                courseService.getCoursesByFeesLessThan(amount));
    }
